...
class GlobalParameters:
    ...
    def __init__(self) -> None:
        ...
        self.default_data_source: str = 'systemtap'  # systemtap, dtrace, oracle
...
class LatencyRecord:
    ...
    def compute_deltas(self, previous: 'LatencyRecord') -> None:
        ...
        for bucket in list(self.data.keys()):
            ...
            # Oracle histograms bucket differently vs SystemTap/DTrace (factor-of-2 difference).
            ...
            elif self.data_source in ('systemtap', 'dtrace'):
                self.intensity_histogram[write_bucket] += (1.5 * delta_count * (2 ** bucket)) / time_factor
            else:
                raise ValueError("Invalid datasource. Use one of: systemtap, dtrace, oracle.")
