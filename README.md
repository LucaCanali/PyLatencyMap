# PyLatencyMap — Latency Heat Maps
...
- **Two complementary views**
    ...
    - **Intensity**: time waited per sec (ms/sec by default)
- **Autotuned axes** for microsecond inputs (default lowest bucket = **128 µs**; labels always shown in **ms**)
- **Pipe-friendly**: read records from `stdin`, render live
- **No DB changes**: works with existing data sources and example connectors
...
label,<free text>
datasource,<systemtap|dtrace|oracle>
...
- `datasource` influences how **Intensity** is approximated from counts:
    ...
    - `systemtap` / `dtrace`: ~ `1.5 * bucket_value * waits`
...
sudo python3 BPF-bcc/block_io_latency_pylatencymap.py | latencymap
```
...
stap -v SystemTap/blockio_rq_issue_pylatencymap.stp 3 | latencymap
```
...
- **v1.3.0** (September 2025) — cleanup, label/unit polish, fixed initial fill, µs-friendly defaults
